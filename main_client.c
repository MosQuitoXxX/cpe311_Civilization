...
int main()
{
	...
	ESP8266_RespBufferReset();	
	LL_mDelay(1000);
	ESP8266_SendCmd((uint8_t*)"AT+CWJAP=\"Redmi_Xince\",\"Nightjester0\"\r\n");
	ESP8266_RespBufferReset();	
...
/*----------------CLIENT------------------------------------------------------*/
	...
	ESP8266_RespBufferReset();
	ESP8266_SendCmd((uint8_t*)"AT+CIPSTART=\"TCP\",\"192.168.43.56\",2759\r\n");
	ESP8266_RespBufferReset();
	
	ESP8266_SendCmd((uint8_t*)"AT+CIPSEND=6\r\n");
	ESP8266_RespBufferReset();
	ESP8266_SendCmd((uint8_t*)"Book");
	ESP8266_RespBufferReset();
	
	while(1)
	{
		(ESP_USART_LOWLEVEL_Recv(resp, idx) != 1)?(idx = (idx + 1) % MAX_RESP_BUFFER_SIZE):(idx);		
		if(strstr((const char*)resp, "Booking"))
		{
			verify = 1 ; //if server sent "Hey!" verify will be 1
		}
